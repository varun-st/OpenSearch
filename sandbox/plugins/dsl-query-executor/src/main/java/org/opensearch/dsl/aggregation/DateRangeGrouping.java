...
/**
 * Expression grouping for date_range bucket aggregation using CASE WHEN.
 * Supports date math expressions (now-7d/d) which are resolved at query planning time.
 */
public class DateRangeGrouping implements ExpressionGrouping {
    private final String aggregationName;
    private final String field;
    private final List<Range> resolvedRanges;
    private final String uniqueId;

    public DateRangeGrouping(String aggregationName, String field, List<Range> ranges, 
                            DateMathParser dateMathParser, LongSupplier nowSupplier, ZoneId timeZone) throws ConversionException {
        this.aggregationName = aggregationName;
        this.field = field;

        this.resolvedRanges = new ArrayList<>();
        for (Range range : ranges) {
            double from = resolveDateMath(range.getFromAsString(), range.getFrom(), dateMathParser, nowSupplier, timeZone);
            double to = resolveDateMath(range.getToAsString(), range.getTo(), dateMathParser, nowSupplier, timeZone);
            resolvedRanges.add(new Range(range.getKey(), from, to));
        }
    ...
    private double resolveDateMath(String dateStr, double numericValue, 
                                   DateMathParser parser, LongSupplier now, ZoneId timeZone) throws ConversionException {
        if (dateStr != null) {
            if (parser == null) {
                throw new ConversionException("Date math parser required for date string: " + dateStr);
            }
            Instant instant = parser.parse(dateStr, now, false, timeZone);
    ...
    public RexNode buildExpression(RelDataType inputRowType, RexBuilder builder) throws ConversionException {
        ...
        for (Range range : resolvedRanges) {
            ...
            String key = range.getKey() != null ? range.getKey() : RangeUtils.generateDateRangeKey(range);
    ...
    public String getProjectedColumnName() {
        return aggregationName + "$$" + field + "$$" + uniqueId + "$$" + "date_range_bucket";
    }
